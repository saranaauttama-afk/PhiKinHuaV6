...
## 11. รีดีไซน์ UI (2026-07-26)

**สิ่งที่พบตอนเปิดดูงานอาร์ตจริง — ต่างจากที่ CLAUDE.md เขียนไว้**

เอกสารเดิมบอกว่างานอาร์ตเป็น "placeholder-grade" ซึ่งไม่จริง ภาพปกกับฉากทั้งสาม
เป็นงานเขียนดินสอ/ถ่าน โทนดินเผา-เขียวมะกอกบนกระดาษเก่า แสงเดียวคือแสงจันทร์อุ่น
มีแดงเลือดหมูเป็นสีเน้นในภาพปก — **นี่คือองค์ประกอบข้อ 1, 4 และ 6 ใน §2 ที่ทำสำเร็จ
ไปแล้วตั้งแต่ต้น** (ภาพวาดแบบหนังสือนิทาน, โทนหม่นตัดไฟอุ่น, ความรู้สึกงานพิมพ์เก่า)

ปัญหาจริงอยู่คนละที่:

| ปัญหา | รายละเอียด |
|---|---|
| **สี UI ไม่เกี่ยวกับงานอาร์ตเลย** | มีสีเขียนตรงๆ 40 กว่าสี เกือบทั้งหมดมาจากพาเลตต์เว็บ (ฟ้า `#93c5fd` ม่วง `#c4b5fd` เขียว `#4ade80`) — เหมือนแดชบอร์ดเว็บแปะบนโปสเตอร์หนังผี |
| **ตัวอักษรผิดแนว** | ตัวเลขสถานะใช้ ChakraPetch ซึ่งเป็นตัวทรงเหลี่ยมแนวเทคโน ขณะที่ THSarabun (ตัวหนังสือเอกสารไทย) อยู่ใน `assets/fonts/` มาตั้งแต่ต้นแต่ไม่เคยถูกโหลด |
| **การ์ด encounter ไม่ตรงกรอบ** | `bgEnNormal.png` ออกแบบช่องไว้ครบ (แถบชื่อ ช่องภาพ ช่องคำบรรยาย แถบล่าง) แต่โค้ดวางด้วย `top:12` / `marginTop:20` ต่อกันไปเรื่อยๆ ตัวหนังสือทับเถาวัลย์ ช่องคำบรรยายว่างเปล่า |
| **แถบสถานะพังบนจอแคบ** | วางด้วย `top:60, left:25, width:280` ตายตัว อิงความกว้างจอเครื่องเดียว |
| **`battle.tsx` ไม่โหลดฟอนต์** | ใช้ฟอนต์แต่ไม่เคยโหลดเอง รอดเพราะปกติเข้าหน้าแผนที่ก่อน แต่ expo-router เปิดตรงเข้าได้ |
| **รูปผีตัวเดียวที่มีเป็นพื้นทึบ** | `phi-krasue.png` มีพื้นเขียวมะกอกติดมา และสีสดกว่าฉากมาก วางบนฉากแล้วเป็นสี่เหลี่ยมลอย |

### สิ่งที่ทำ

- `app/theme.ts` — พาเลตต์ที่**ดึงจากภาพจริง** ไม่ได้คิดใหม่ กติกาคือ
  **สีเน้นมีสองสีเท่านั้น**: ทองแสงจันทร์ กับ แดงเลือดหมู (ใช้กับ HP/อันตรายเท่านั้น)
  ค่าสถานะแยกกันด้วยป้ายชื่อ ไม่ใช่ด้วยการแจกสีคนละสี — การแจกสีรุ้งคือสิ่งที่ทำให้
  หน้าจอดูเป็นเว็บแอป
- `app/useAppFonts.ts` — โหลดฟอนต์ที่เดียว เพิ่ม THSarabun สำหรับเนื้อเรื่อง
  เลิกใช้ ChakraPetch ทั้งโปรเจกต์
- `app/components/Panel.tsx` — กรอบกระดาษ + ปุ่ม + ม่านทับฉาก ทำด้วยโค้ด
  ไม่ต้องรออาร์ตใหม่ ได้อาร์ตกรอบจริงเมื่อไหร่แก้ไฟล์เดียวทุกหน้าเปลี่ยนตาม
- `encounterFrame` ใน theme — สัดส่วนช่องที่**วัดจากไฟล์ภาพ** การ์ดวางเนื้อหา
  ตามช่องจริงแล้ว เปลี่ยนขนาดการ์ดทุกอย่างขยับตามสัดส่วนเอง
- `ShopView.tsx` เขียนใหม่ทั้งไฟล์ — เดิมเป็นภาษาอังกฤษล้วน สี 37 สี และผสม
  nativewind กับ inline style สลับกัน พฤติกรรมทุกอย่างยกมาครบ เปลี่ยนแค่หน้าตากับภาษา
- `test/design-system.test.ts` — **กันสีนอกพาเลตต์ไหลกลับเข้ามา** พร้อมลิสต์
  `NOT_YET_MIGRATED` ที่ต้องลดลง ห้ามเพิ่ม และเช็คว่าฟอนต์ที่ใช้ถูกโหลดจริงทุกตัว

### ที่ยังไม่ได้ทำ

หน้าจอในโฟลเดอร์ `battle/` กับ `StartPage` / `DeckView` / `EventView` ยังไม่ได้ย้าย
เข้าระบบ — อยู่ในลิสต์ `NOT_YET_MIGRATED` ของเทสต์ ตั้งใจแยกเป็นรอบถัดไป
เพราะหน้าต่อสู้มีอนิเมชั่นผูกกับเลย์เอาต์ ต้องแก้ระวังกว่าหน้าอื่น

## 12. รีดีไซน์รอบสอง — หน้าต่อสู้และที่เหลือ (2026-07-26)

ลิสต์ `NOT_YET_MIGRATED` ใน `design-system.test.ts` **ว่างแล้ว** — ทุกหน้าจอในเกม
ใช้พาเลตต์เดียวกันหมด

การไล่เปลี่ยนสีทีเดียวทั้งโปรเจกต์มีผลข้างเคียงที่มองไม่เห็น จึงเพิ่มตาข่ายอีกชั้น:

**`a ? X : X`** — สีเดิมสองสีที่ต่างกัน (ฟ้ากับเขียว) ถูกแม็ปไป token เดียวกัน
ternary จึงเหลืออยู่แบบไม่มีความหมาย โค้ดยังทำงาน เทสต์สียังผ่าน แต่ของสองอย่าง
ที่เคยแยกด้วยตาได้กลายเป็นสีเดียวกัน **เจอจริงสามจุด**:

- ป้ายเด้ง Block กับ Energy → แก้เป็นทองหม่น/ทองสว่าง
- ชนิดการ์ดในสำรับ (วิชา/เครื่องราง) → แยกด้วยความสว่าง
- ปุ่มในจอแพ้ ตอนกดกับไม่กดสีเดียวกัน = ไม่มีฟีดแบ็กว่าแตะโดน

เทสต์ `ไม่มีทางเลือกสีที่สองทางให้ผลเหมือนกัน` ดักเคสนี้แล้ว (ยืนยันว่าแดงจริง)

### โค้ดตายที่เจอเพิ่ม

- `Temp.tsx`, `EnemyCardPlay.tsx` — ไม่มีใครเรียก
- `resolveShops()` / `callOrLog()` ใน `map_pages.ts` — ประกาศไว้แต่**ไม่เคยถูกเรียก**
  และเป็นทางเดียวที่จะไปถึงระบบ event เก่า แปลว่า `EventView` เข้าไม่ถึงในการเล่นปกติ
  (เหลือเฉพาะผ่านคำสั่ง QA) — ตอนนี้ระบบเหตุการณ์จริงคือ `StoryEventView`

**เกือบพลาด:** `Card.tsx` เกือบถูกลบเพราะ grep แรกหาด้วย path เต็ม แต่ `PlayerHand`
เรียกด้วย `'../Card'` — ตรวจซ้ำด้วย relative path ก่อนจึงเจอ

**TypeScript ช่วยไว้:** ตอนแปลปุ่มเป็นไทย การแทนคำแบบตรงตัวกินชื่อคำสั่งไปด้วย
(`UnequipToDeck` → `ถอดToDeck`) tsc จับได้ทันที

## 13. บั๊กเปิดแอปแล้วค้างหน้าเปล่า (2026-07-26)

**อาการ:** เปิดเกมมาแล้วเจอหน้าแผนที่ที่ไม่มีอะไรเลย — ไม่มีแถบเส้นทาง ไม่มีการ์ดทางแยก
มีแต่พื้นหลังหนองน้ำกับแถบสถานะ กดอะไรไม่ได้ ไปไหนไม่ได้

**ต้นเหตุ:** `makeEmptyState()` ใน `gameStore.ts` ตั้ง `phase: 'menu'`
แต่ `index.tsx` เช็คว่า `phase === 'start'` ถึงจะโชว์หน้าเริ่มเกม
`'menu'` จึงร่วงผ่านทุกเงื่อนไขไปตกที่ **หน้าแผนที่ซึ่งเป็น fallback ท้ายสุด**
ตอนนั้นยังไม่มี `journey` และ `pages` เลย จอจึงว่างเปล่า

ค่าที่เห็นบนแถบสถานะยืนยันตรงนี้ — HP 50/50, ทอง 80, EXP 0/20 คือค่าจาก
`makeEmptyState()` เป๊ะ ไม่ใช่เซฟที่โหลดมา

**ทำไมเทสต์ 239 ตัวไม่จับ:** ทุกตัวยิงเข้า engine ตรงๆ ผ่าน `applyCommand`
ไม่มีตัวไหนถามว่า "state แบบนี้ UI จะเลือกแสดงหน้าไหน" ตรรกะเลือกหน้าเป็น
if-chain ฝังอยู่กลาง JSX ซึ่งทดสอบไม่ได้เลย

### สิ่งที่แก้

1. `makeEmptyState()` ใช้ `baseNewState('')` ตัวเดียวกับ engine — ได้ `phase: 'start'`
   ที่ถูกต้อง และตัดข้อมูลการ์ด 6 ใบที่ก๊อปมาแปะไว้ในไฟล์ store ทิ้ง (มันเก่ากว่า
   `cards.json` ไปแล้วด้วย)
2. `app/screenRouter.ts` — ย้ายตรรกะเลือกหน้าออกมาเป็นฟังก์ชันบริสุทธิ์
   `'menu'` กับ `'start'` ถือว่าเหมือนกัน (ทั้งคู่แปลว่ายังไม่ได้อยู่ในรัน)
3. `mapIsReady()` + จอทางออก — ถ้าหลุดมาถึงหน้าแผนที่โดยไม่มีเส้นทาง
   (เช่นเซฟเก่าที่บันทึกก่อนระบบเส้นทาง) จะเจอข้อความบอกพร้อมปุ่มออกเดินทางใหม่
   แทนที่จะเป็นจอเปล่า
4. `test/screen-router.test.ts` — 12 เทสต์ รวมข้อที่**อ่าน state จริงจาก store**
   ไม่ใช่จาก `baseNewState` เพราะบั๊กนี้เกิดจากสองที่นั้นไม่ตรงกัน
   (ยืนยันแล้วว่าใส่บั๊กกลับเข้าไปเทสต์แดงจริง)

**บทเรียน:** หน้าแผนที่เป็น fallback ท้ายสุดของ if-chain ทำให้ phase ไหนที่ไม่มีใคร
รับผิดชอบตกมาที่นี่หมดโดยไม่มีเสียงเตือน — ตอนนี้เลือกหน้าด้วย switch ที่ครบทุกค่า
